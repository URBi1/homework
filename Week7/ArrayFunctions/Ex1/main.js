  ...
  emailAndCompanyName = users.map(user=>{
    return {email: user.email, companyName: user.company.name}
  })

  console.log(emailAndCompanyName)

  let usersWithZipStartingWith5 = users.filter(user => {
    return user.address.zipcode.startsWith("5");
});

console.log(usersWithZipStartingWith5);

let usersWithZipStartingWith5OnlyId = users.filter(user => {
    return user.address.zipcode.startsWith("5")
}).map(user=>user.id)

console.log(usersWithZipStartingWith5OnlyId);

let nameStartC = users.map(user => user.name).filter(name => name.startsWith("C"))
...
let userWithSuite950 = users.find(user => user.address.suite === "Apt. 950");
